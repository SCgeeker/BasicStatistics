...
knitr::kable(
  matrix(c(63,90,153,20,132,152),nc=2, dimnames = list(c("未回答","有回答","總人數"), c("無提示","有提示"))), 
  booktabs = TRUE,
...
```{r chi-independent-simulation, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, results='asis',fig.cap="獨立性檢定抽樣分佈(黑色柱狀圖);自由度為1的卡方分佈(紅色線條);自由度為2的卡方分佈(藍色線條)"}
...
計算差異平方和，會發現抽樣分佈與自由度為1的卡方分佈之差異(`r round(sum((sampling_density - dchisq(1:length(sampling_density), df=1) )[-1]^2),3)`)，小於抽樣分佈與自由度為2的卡方分佈之差異(`r round(sum((sampling_density - dchisq(1:length(sampling_density), df=2) )[-1]^2),3)`)。我們可以將上述公式改寫為以下形式，其中O代表各細格實際次數，E代表各細格期望次數：
...
```{r independent-continuous-populations, echo=FALSE, message=FALSE, warning=FALSE,  out.width="50%",fig.show='hold',fig.cap="獨立樣本t檢定母群體機率函數"}
...
```{r independent-continuous-sampling, echo=FALSE, message=FALSE, warning=FALSE, paged.print=TRUE,results='asis',fig.cap="兩組樣本平均數差異值的抽樣分佈"}
...
```{r independent-continuous-sampling-qq, echo=FALSE, message=FALSE, warning=FALSE, paged.print=TRUE,results='asis',fig.cap="兩組樣本平均數差異值的抽樣分佈分位圖"}
...
```{r independent-continuous-sampling-dist, echo=FALSE, message=FALSE, warning=FALSE, paged.print=TRUE,fig.cap="兩組樣本平均數差異值的標準化分數 vs. 常態分佈 vs. t分佈"}
...
計算曲線之間的**差異平方和**，可發現兩組樣本平均數差異值的標準化分數與t分佈的差異`r round(sum((sim_stat$y - dt(seq(-4,4,1/length(sim_stat$x)), df=(counter_n+clock_n-2)) )^2),2)`，小於與常態分佈的差異`r round(sum((sim_stat$y - dnorm(seq(-4,4,1/length(sim_stat$x)) ) )^2),2)`。
...
```{r independent-power, echo=FALSE, message=FALSE, warning=FALSE, paged.print=TRUE,fig.cap="達到指定考驗力之獨立樣本t檢定效果量，所需要的樣本數分析。"}
knitr::include_graphics("image/unit07_independent_power.png")
```
...
```{r paired2-power, echo=FALSE, message=FALSE, warning=FALSE, paged.print=TRUE,fig.cap="達到指定考驗力之相依樣本t檢定效果量，所需要的樣本數分析。"}
knitr::include_graphics("image/unit07_paired_power.png")
```
