...
```{r onefactor-sampling, echo=FALSE, message=FALSE, warning=FALSE, paged.print=TRUE,fig.cap="適合度考驗的抽樣分佈。 虛無假設(紅)與對立假設(藍)"}
## Testing of one categorical variable
Groups <- 5    # Number of groups
Total <- 200  # Number of total observations
Exp_p <- c(.1,.2,.2,.2,.3) # Probability of expected value
...
Title <- paste0("適合度考驗的抽樣分佈 \n 總計人數為", Total, "， 自由度為",DF) #
hist(chi0,xlab="統計數",ylab="機率密度", density = TRUE, freq = FALSE, main=Title,xlim=c(0,max(chi1)), ylim=c(0,0.2), col="red")   #
lines(dchisq(seq(min(chi0),max(chi0),by=1/1000),df=DF) ~ seq(min(chi0),max(chi0),by=1/1000), col="grey" ) #
hist(chi1, col = "blue", density = TRUE, freq = FALSE, add=TRUE)      #
abline(v=qchisq(.95,df=DF),col="black") #
```
...
圖\@ref(fig:onefactor-sampling)內的垂直黑線，代表以自由度4的卡方機率函數，判斷水準是.05的統計數`r round(qchisq(.95, df=4),2)`。虛無假設抽樣分佈超過此線的比例，代表本範例的型一錯誤率估計值，運用[jamovi示範檔案](https://osf.io/kcswx/)，讀者可發現無論如何改變期望次數的組合，虛無假設抽樣分佈樣態改變幅度不大，型一錯誤率都會維持在.05左右。所以分析者可以根據實際需求，調整判斷水準也能保證認定有差異的結論出錯之機率，不會超過型一錯誤率。
...
```{r jamovi-goodnessoffit, echo=FALSE, message=FALSE, warning=FALSE, paged.print=TRUE,fig.cap="jamovi適合度考驗操作示範"}
knitr::include_graphics("images/unit07_jamovi_goodnessoffit.png")
```
...
```{r JASP-goodnessoffit, echo=FALSE, message=FALSE, warning=FALSE, paged.print=TRUE,fig.cap="jamovi適合度考驗操作示範"}
knitr::include_graphics("images/unit07_JASP_goodnessoffit.png")
```
...
```{r inde-contigency, echo=FALSE, message=FALSE, warning=FALSE, paged.print=TRUE,fig.cap="美國大學生平常從事的運動強度與日常攝取水果數量"}
knitr::include_graphics("images/unit07_jamovi_twofactor_contigencytable.png")
```
...
```{r chi-independent-simulation, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, results='asis',cache=TRUE,fig.cap="獨立性考驗的抽樣分佈。 虛無假設(紅)與對立假設(藍)"}
...
Title <- paste0("獨立性考驗的抽樣分佈 \n 總計人數為", Total, "， 自由度為",DF)
hist(chi0,xlab="統計數",ylab="機率密度", density = TRUE, freq = FALSE, main=Title,ylim=c(0,max(dchisq(seq(min(chi0),max(chi0),by=1/1000),df=DF))),xlim = c(0,max(chi1)), col="red")   #
...
```{r independence-jamovi, out.width='80%', fig.cap="卡方檢定獨立性考驗操作設定示範：jamovi", fig.align='center', echo=FALSE}
knitr::include_graphics("images/unit07_jamovi_contigency.png")
```


```{r independence-JASP, out.width='80%', fig.cap="卡方檢定獨立性考驗操作設定示範：JASP", fig.align='center', echo=FALSE}
knitr::include_graphics("images/unit07_JASP_contigency.png")
```
...
```{r independent-continuous-populations, echo=FALSE, message=FALSE, warning=FALSE, cache=TRUE,  out.width="50%",fig.show='hold',fig.cap="kitchen roll研究資料分組資料"}
require(tidyverse)
data <- read.csv(file = "./codes/unit07_kitchen roll.csv")
data %>% select(Rotation,mean_NEO) %>%
  ggplot(aes(x=Rotation, y = mean_NEO)) +
  geom_violin(draw_quantiles = c(0.25, 0.5, 0.75)) +
  geom_jitter(height = 0, width = 0.1)
```

順時針組的評分平均值為`r round(mean(subset(data,Rotation=="clock")$mean_NEO),2)`、樣本標準差為`r round(sd(subset(data,Rotation=="clock")$mean_NEO),2)`；逆時針組的評分平均值為`r round(mean(subset(data,Rotation=="counter")$mean_NEO),2)`、樣本標準差為`r round(sd(subset(data,Rotation=="counter")$mean_NEO),2)`。根據原始資料，使用拔靴法分別製造兩批各10000筆模擬平均數，形成圖\@ref(fig:independent-groups-sample)的平均數抽樣分佈，可見模擬平均數抽樣分佈的平均值近真實資料的統計值，模擬分佈的樣本標準誤是真實資料樣本標準差的$\frac{1}{\sqrt{`r sum(data$Rotation=="clock")` }}$與$\frac{1}{\sqrt{`r sum(data$Rotation=="counter")` }}$。

```{r independent-groups-sample, echo=FALSE, message=FALSE, warning=FALSE,cache=TRUE, paged.print=TRUE,results='asis',fig.cap="順時針(紅色)與逆時針(藍色)的樣本平均值抽樣分佈。"}
data <- read.csv(file = "./codes/unit07_kitchen roll.csv") #
counter_data <- subset(data,Rotation == "counter")$mean_NEO #
clock_data <- subset(data,Rotation == "clock")$mean_NEO #
#
counter_N <- length(counter_data) #
clock_N <- length(clock_data) #
#
counter_DF <- counter_N - 1 #
clock_DF <- clock_N - 1 #
#
...
i = 10000  #
while (i > 0) {                          #
  clock_sim_means <- c(clock_sim_means,   mean(sample(clock_data,clock_N, replace = TRUE)) )  # 
  counter_sim_means <- c(counter_sim_means,
  mean(sample(counter_data,counter_N, replace = TRUE)) )   #
  i = i - 1   #
}  #
## Draw sampling means
Title = paste("順時針平均值：", round(mean(clock_sim_means),2), "; 樣本標準誤：", round(sd(clock_sim_means),2),"\n逆時針平均值：", round(mean(counter_sim_means),2), "; 樣本標準誤：", round(sd(counter_sim_means),2))   #
hist(clock_sim_means,density = TRUE, freq = FALSE, main=Title, xlab = "平均評分",ylab="機率密度",ylim=c(0,7),col="red")  #
hist(counter_sim_means,col="blue",density = TRUE, freq = FALSE, add=TRUE)   #
#abline(v=qt(alpha,df=(counter_DF + clock_DF)))
```

兩組的合併標準差為`r round(sqrt((53*(.473)^2+47*(.496)^2)/(53+47)),3)`，加權標準誤為`r round(sqrt((53*(.473)^2+47*(.496)^2)/(53+47))*sqrt(1/54+1/48),3)`，接近圖\@ref(fig:independent-hypothesis-sampling)的對立與虛無假設抽樣分佈標準誤。

```{r independent-hypothesis-sampling, echo=FALSE, message=FALSE, warning=FALSE,cache=TRUE, paged.print=TRUE,results='asis',fig.cap="平均值差異的5抽樣分佈：虛無假設(紅色)與對立假設(藍色)"}
H1 <- sample(counter_sim_means,10000,replace = TRUE) - sample(clock_sim_means,10000,replace = TRUE)
H0 <- sample(clock_sim_means,10000,replace = TRUE) - sample(clock_sim_means,10000,replace = TRUE)
#sd(H1)/sqrt(1/clock_N+1/counter_N)
Title = paste("虛無假設平均值：", round(mean(H0),2), "; 樣本標準誤：", round(sd(H0),3),"\n對立假設平均值：", round(mean(H1),2), "; 樣本標準誤：", round(sd(H1),3))
hist(H0,density = TRUE, main=Title, freq = FALSE, xlab = "平均評分",ylab="機率密度",ylim=c(0,7),col="red")
hist(H1,density = TRUE, freq = FALSE, col="blue", add=TRUE)
lines(x=sort(H0),y=dnorm(sort(H0),mean=mean(H0),sd=sd(H0)),col="red")
lines(x=sort(H1),y=dnorm(sort(H1),mean=mean(H1),sd=sd(H1)),col="blue")
abline(v=qnorm(.975,mean=mean(H0),sd=sd(H0)))
```

兩種抽樣分佈皆符合常態分佈，以雙尾檢定的判斷水準.05為界，估計型一錯誤率為`r round(mean(H0 > qnorm(.975,mean=mean(H0),sd=sd(H0))),3)`，型二錯誤率為`r round(mean(H1 < qnorm(.975,mean=mean(H0),sd=sd(H0))),3)`。模擬數據的剖析確定這筆資料符合**常態分佈假設**，可採用變異等量t檢定分析策略。除了估計的型一錯誤率接近判斷水準，更值得注意的是型二錯誤率估計高達0.9。因為這筆資料是重現他人研究，值得探究這份研究低考驗力的原因。
...
knitr::include_graphics("images/unit07_JASP_independent_t.PNG")
```


```{r jamovi-independent-t-operation, out.width='80%', fig.cap="jamovi獨立樣本雙尾t檢定分析操作設定示範。", fig.align='center', echo=FALSE}
knitr::include_graphics("images/unit07_jamovi_independent_t.png")
```
...
```{r independent-power, echo=FALSE, message=FALSE, warning=FALSE, fig.align='center', fig.cap="達到指定考驗力之獨立樣本t檢定效果量，所需要的樣本數分析。"}
knitr::include_graphics("images/unit07_independent_power.png")
```
...
```{r paired2-power, echo=FALSE, fig.align='center', fig.cap="達到指定考驗力之相依樣本t檢定效果量，所需要的樣本數分析。", message=FALSE, warning=FALSE}
knitr::include_graphics("images/unit07_paired_power.png")
```
