...
由表\@ref(tab:sim-tea-p-values)可知，10000次實驗裡，大於.05的結果有5筆，大於.01的結果總計37筆。許多運用虛無假設顯著性檢定的分析，採用.05或.01為檢定水準的理由來自類似的模擬程序。研究者要採用那一種檢定水準，最佳的策略是了解要測試的虛無假設與對立假設性質。理解計算的與模擬的機率分佈，我們可以歸納兩者性質如下：
