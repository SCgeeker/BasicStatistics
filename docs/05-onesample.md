...
由表\@ref(tab:sim-tea-p-values)可知，10000次實驗裡，大於.05的結果有8筆，大於.01的結果總計27筆。許多運用虛無假設顯著性檢定的分析，採用.05或.01為檢定水準的理由來自類似的模擬程序。研究者要採用那一種檢定水準，最佳的策略是了解要測試的虛無假設與對立假設性質。理解計算的與模擬的機率分佈，我們可以歸納兩者性質如下：
...
相對於型一錯誤率，型二錯誤率指根據檢定水準判斷實際得到的p值，否定對立假設卻不正確的最大機率。能經得起長期考驗的研究結果，型一與型二錯誤率應該越小越好。設定檢定水準體現研究者對型一錯誤率的期許，型二錯誤率與研究者肯定對立假設的正確率互為補集，在統計學裡後者被稱為**考驗力(Power)**，也就是表\@ref(tab:sim-alpha)的0.63。除了透過儘可能最小的檢定水準得到顯著結果，經得起考驗的研究結果也要有儘可能最大的考驗力。我們在第\@ref(paired)單元，將進一步認識什麼是考驗力，以及如何確保研究結果有起碼的考驗力。
