...
```{r big5-smalln, echo=FALSE, message=FALSE, warning=FALSE, paged.print=TRUE, results='asis',fig.cap="設定樣本數為十二位學生，五大人格特質相關係數的抽樣分佈。"}
require(dplyr)
...
plot(cors_d,
     main = paste("皮爾森相關係數之抽樣分佈 \n 母數 = ",phi0,", 樣本數 = ",sample_n),
     xlab = "皮爾森相關係數",
     ylab = "機率密度",
     xlim = c(-2,0.5))
...
以 @DolanTestingMeasurementInvariance2009 的五大人格特質資料 為例，如果盡責性與情緒不穩定性相關係數的母群體是$\rho = -0.368$，只收集12位學生的資料，模擬10000筆樣本相關係數形成的抽樣分佈如同圖\@ref(fig:big5-smalln)呈現負偏態的黑色曲線，與分佈中心是-0.368的常態分佈(綠色曲線)有明顯的差異。10000樣本相關係數經過費雪轉換，形成的抽樣分佈(紅色曲線)與平均數是`r round(mean(F_cors1),2)`及標準誤是`r round(sd(F_cors1),2)`的常態分佈(藍色曲線)完全契合，此標準誤相當接近以公式$\frac{1}{\sqrt{n-3}}$計算的`r round(1/sqrt(12-3), 2)`。


```{r big5-largen, echo=FALSE, message=FALSE, warning=FALSE, paged.print=TRUE, results='asis', fig.cap="設定樣本數為五百位學生，五大人格特質相關係數的抽樣分佈。"}
require(dplyr)
...
將樣本數增加至如至500人，樣本相關係數的抽樣分佈就會契合對稱的機率分佈，如圖\@ref(fig:big5-largen)代表抽樣分佈的黑色曲線，完全符合抽樣分佈平均數`r round(mean(cors2),2)`與標準誤`r round(sd(cors2),2)`的常態分佈(綠色曲線)。費雪轉轉換後的抽樣分佈(紅色曲線)雖然也符合常態分佈(藍色曲線)，但是估計的期望值`r round(mean(F_cors2),2)`明顯不同於母群體。由此例可知，運用相關係數分析資料的研究，要獲得不偏的相關係數估計值，收集的樣本數要達到上百筆。
...
```{r big5-hypo, echo=FALSE, message=FALSE, warning=FALSE, paged.print=TRUE, results='asis',fig.cap="樣本數為200，五大人格特質之相關係數是否為0的假設檢定。"}
require(dplyr)
sample_n <- 200 ## Setting of sample size
phi0 <- -0.368    ## Setting of correlation
...
```{r corr-crit, echo=FALSE, message=FALSE, warning=FALSE, paged.print=TRUE, results='asis',fig.cap="檢定水準為.05的雙側t檢定之判斷水準與樣本數(自由度)之變化"}
n <- 1:100
t <- qt(.975,n)
r <- t/sqrt(n-2+t^2)
plot(r~n,type="l",xlab="自由度(樣本數-2)",ylab="t檢定決定水準")
```
...
```{r corr-reasons, out.width='80%', fig.cap="相關係數的各種意義。Thomas Lumley提供(推特帳號：tslumley)。", fig.align='center', echo=FALSE}
knitr::include_graphics("images/unit08_cor_reasons.png")
```
...
```{r reg-sample, echo=FALSE, message=FALSE, warning=FALSE, paged.print=TRUE, results='asis',fig.cap="樣本數為100，迴歸係數之抽樣分佈。"}
